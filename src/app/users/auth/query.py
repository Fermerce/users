...
from src.app.users.auth import model


class AuthTokeQuery(BaseQuery[model.AuthToken]):
    def __init__(self):
    ...
    async def create_obj(
        ...
        if token_id:
            ...
            if return_token:
                return result
            return
        new_auth_token = await super().create_obj(
            dict(
                access_token=access_token,
                refresh_token=refresh_token,
                ip_address=user_ip,
...
auth_token_query = AuthTokeQuery()
