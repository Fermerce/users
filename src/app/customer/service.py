...
async def create(data_in=schema.ICustomerIn):
    check_customer = await customer_repo.get_by_attr(attr=dict(email=data_in.email), first=True)
    if check_customer:
    ...
    if new_customer:
        ...
        get_permission = await permission_repo.get_by_attr(attr=dict(name="customer"), first=True)

        if not get_permission:
            get_permission = await permission_repo.create(dict(name="customer"), expunge=False)
...
async def reset_password_link(
    ...
    customer_obj = await customer_repo.get_by_attr(attr=dict(email=data_in.email), first=True)
