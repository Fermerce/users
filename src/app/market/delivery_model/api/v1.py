...
from src.app.market.delivery_model import schema, service
...
@router.get(
    "/{delivery_mode_id}",
    # response_model=schema.IDeliveryModeOut,
    # dependencies=[Depends(require_super_admin_or_admin)],
)
async def get_delivery_mode(delivery_mode_id: uuid.UUID) -> schema.IDeliveryModeOut:
    return await service.get(delivery_mode_id=delivery_mode_id)
...
async def update_delivery_mode(
    delivery_mode_id: uuid.UUID, delivery_mode: schema.IDeliveryModeIn
) -> schema.IDeliveryModeOut:
...
async def delete_delivery_mode(delivery_mode_id: uuid.UUID) -> None:
    return await service.delete(delivery_mode_id=delivery_mode_id)
